...
from PyQt6.QtGui import QScreen, QGuiApplication, QPixmap, QImage, QMouseEvent, QCursor
...
class RegionSelectorDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.__instruction_label = QLabel(
...
class ScreenRecorderApp(QMainWindow):
    ...
    def __start_recording(self) -> None:
        ...
        if not self.__screen_region:
            QMessageBox.warning(
                self, "Warning", "Please select a recording region first!"
            )
            return
        ...
        pid = os.getpid()
        video_filename = (
            f"temp_screen_recording_{pid}.{self.__format_combo.currentText().lower()}"
        )
        audio_filename = f"temp_audio_{pid}.wav"

        # Initialize video writer with initial frame rate
        self.__video_writer = cv2.VideoWriter(
            video_filename, fourcc, 20.0, (width, height)  # Initial default frame rate
        )
    ...
    def __stop_recording(self) -> None:
        ...
        if save_path:
            ...
            try:
                ...
                QMessageBox.warning(
                    self, "Error", f"Could not save recording: {str(e)}"
                )
